...
export const customerApi = {
  ...
  getMe: async (): Promise<Customer> => {
    const response = await api.get<ApiResponse<Customer>>('/v1/customers/me');
  ...
  updateProfile: async (data: UpdateProfileRequest): Promise<Customer> => {
    const response = await api.put<ApiResponse<Customer>>('/v1/customers/me', data);
    return response.data;
  },
  ...
  changePassword: async (data: PasswordChangeRequest): Promise<void> => {
    await api.put('/v1/customers/me/password', data);
  },
  ...
  getAddresses: async (): Promise<Address[]> => {
    const response = await api.get<ApiResponse<Address[]>>('/v1/customers/me/addresses');
    return response.data;
  },
  ...
  addAddress: async (data: AddressRequest): Promise<Address> => {
    const response = await api.post<ApiResponse<Address>>('/v1/customers/me/addresses', data);
    return response.data;
  },
  ...
  withdraw: async (data: WithdrawRequest): Promise<void> => {
    await api.delete('/v1/customers/me', data);
  },
};
